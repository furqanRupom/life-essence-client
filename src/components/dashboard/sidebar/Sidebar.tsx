...
const Sidebar: React.FunctionComponent<ISidebarProps> = ({ isSidebarOpen, setIsSidebarOpen }) => {
    ...
    return (
        <div className="z-50 min-h-screen h-full md:h-0 border border-coral-50">
            <nav
            ...
            >
                ...
                <ul className="p-4 space-y-2">
                    {sidebarMenus.map((menu, index: any) => (
                        <motion.li
                            key={index}
                            whileTap={{ scale: 0.95 }}
                            className={`hover:bg-coral-50 text-default-60 ${pathname === menu.route ? "bg-coral-50 text-coral-400" : ""} px-3 py-1 rounded-xl `}
                        >
                            <Link href={menu.route} className="text-deafult-600 items-center gap-3 text-lg py-1 hover:text-blue-500 flex ">
